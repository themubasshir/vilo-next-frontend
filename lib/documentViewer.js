...
export function getDocumentEditMode({ filename = "", mediaType = "" } = {}) {
  const normalizedType = normalizedMediaType(mediaType);
  const extension = filenameExtension(filename);

  if (extension === "docx" || (normalizedType === DOCX_MEDIA_TYPE && extension !== "doc")) {
    return "onlyoffice";
  }
  // A filename signal is required: text/plain is too commonly used as a
  // generic browser fallback to classify arbitrary stored bytes as editable.
